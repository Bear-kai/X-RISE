...
from dataset.dataset_utils import dict_apply
...
def train(args):
    ...
    cprint(f"aug_cond_seq: {args.aug_cond_seq}", "magenta")
    ...
    ckpt_dir = pathlib.Path(os.path.join(RISE_ROOT, 'checkpoints_X', ckpt_name))
        ...
        if do_eval:
            ...
            with torch.no_grad():
                ...
                for data_val in pbar_val:
                    # cloud data processing
                    data_val = dict_apply(data_val, lambda x: x.to(device=device))
                    ...
                    pose_data_val = data_val['pose_normalized']
                    cloud_data_val = ME.SparseTensor(cloud_feats_val, cloud_coords_val)
                    data_val['cloud'] = cloud_data_val
                    # forward
                    loss_val = policy(data_val, action_data_val, pose_data_val, batch_size = action_data_val.shape[0])
...
def arg_parse():
    ...
    parser.add_argument('--aug_cond_seq', action = 'store_true', help = 'whether to add conditional sequence augmentation')
