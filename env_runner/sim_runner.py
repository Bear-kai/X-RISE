...
class RobotRunner(BaseRunner):
    ...
    def get_action(self, policy, observaton=None) -> bool:
        ...
        with torch.no_grad():
            ...
            cloud_data = ME.SparseTensor(cloud_feats.to(device), cloud_coords.to(device))

            # predict
            pred_raw_action = policy(cloud_data, actions=None, batch_size=1).squeeze(0)
            
            if policy.have_normalizer:
                # dp3 data style
                action = policy.normalizer['action'].unnormalize(pred_raw_action).cpu().numpy()
            else:
                # rise data style
                action = unnormalize_and_transform(pred_raw_action).cpu().numpy()
