...
def plot_history_eval(eval_history, num_epochs, ckpt_dir, seed):
    # save training curves
    plt.figure()
    plt.plot(np.linspace(0, num_epochs, len(eval_history)), eval_history, label = 'eval')
    plt.tight_layout()
    plt.legend()
    plt.title("loss")
    plt.savefig(os.path.join(ckpt_dir, f'eval_seed_{seed}.png'))
