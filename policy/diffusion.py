...
from policy.diffusion_modules.conditional_unet1d import ConditionalUnet1D         # 50.94M
# from policy.diffusion_modules.conditional_unet1d_copy import ConditionalUnet1D  # 56.3M; minor change
...
class DiffusionUNetPolicy(nn.Module):
    ...
    def compute_loss(self, readout, actions):
        batch_size = readout.shape[0]   # (B, D=512)

        # handle different ways of passing observation
        local_cond = None
        global_cond = None
        trajectory = actions            # (B, L=20, D=10)
        cond_data = trajectory
        # assert readout.shape[0] == batch_size * self.n_obs_steps
        # reshape back to B, Do
        global_cond = readout.reshape(batch_size, -1) # (B, D=512)
        ...
        pred = self.model(noisy_trajectory, timesteps, 
            local_cond=local_cond, global_cond=global_cond)  # (B,L=20,D=10)
        ...
        # loss = reduce(loss, 'b ... -> b (...)', 'mean')  # (B,L=20,D=10) --> (B,LxD=200)
        loss = loss.mean()
        return loss
