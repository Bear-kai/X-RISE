...
class Sparse3DEncoder(torch.nn.Module):
    def __init__(self, input_dim = 6, output_dim = 512, num_obs=1):
        super().__init__()
    ...
    def forward(self, sinput, max_num_token=100, batch_size=24):
        ...
        '''
        soutput = self.cloud_encoder(sinput)    # eg. (388352, 6) --> feat_F:(900, 512); coord_C:(900,4)
        feats_batch, coords_batch = soutput.F, soutput.C
        feats_list = []
        coords_list = []

        # original code: does not support num_obs>1
        # for i in range(batch_size):
        #     mask = (coords_batch[:,0] == i)
        #     feats_list.append(feats_batch[mask])
        #     coords_list.append(coords_batch[mask])

        # now support num_obs>1
        max_num_token *= self.num_obs
        for i in range(batch_size):                   
            j_start, j_end = i*self.num_obs, (i+1)*self.num_obs
            mask = torch.zeros_like(coords_batch[:,0], dtype=torch.bool)
            for j in range(j_start, j_end):
                mask |= (coords_batch[:,0] == j)
            feats_list.append(feats_batch[mask])
            coords_list.append(coords_batch[mask])  

        pos_list = self.position_embedding(coords_list)

        # prepare inputs to transformer
...
class SparsePositionalEncoding(nn.Module):
    ...
    def _init_position_vector(self):       
        x_steps = y_steps = self.num_pos_feats // 3         # eg. 170
        z_steps = self.num_pos_feats - x_steps - y_steps    # eg. 172
        ...
        pos_x_vector = xyz_embed / x_dim_t  # (800,1) / (170) --> (800,170)
        pos_y_vector = xyz_embed / y_dim_t
        pos_z_vector = xyz_embed / z_dim_t
        self.pos_x_vector = torch.stack([pos_x_vector[:,0::2].sin(), pos_x_vector[:,1::2].cos()], dim=2).flatten(1)  # eg.(800,170)
    ...
    def forward(self, coords_list):
        ...
        for coords in coords_list:
            ...
            pos_x = self.pos_x_vector.to(coords.device)[coords[:,0]]    # eg. (56,3) --> (56,170)
            pos_y = self.pos_y_vector.to(coords.device)[coords[:,1]]
            pos_z = self.pos_z_vector.to(coords.device)[coords[:,2]]
            pos = torch.cat([pos_x, pos_y, pos_z], dim=1)               # eg. (56,512) 
            pos_list.append(pos)
        return pos_list