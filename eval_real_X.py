...
from policy.policy import XRISE
...
from env_runner.real_runner_X import RobotRunner, WrapperPolicyRunner
...
default_args = edict({
    ...
    "num_inference_step": 16,   # the number of execution  steps, range: [num_action, horizon], ensure the policy outputs full horizon actions
    "num_action": 8,            # length of conditional sequence, keep the same with the value in training stage
    "num_pose": 8,              # length of conditional sequence, keep the same with the value in training stage
    "action_dim": 10,
...
def create_input(colors, depths, cam_intrinsics, projector, cam_id, voxel_size = 0.005):
    ...
    obs = dict()
    obs['pointcloud'] = cloud[:,:3]   # exclude rgb
...
def reset_func(model):              
    ...
    # such as the observation window.
...
def evaluate():
    ...
    env_runner = RobotRunner(n_obs_steps=1, # args.num_obs, 
                            n_action_steps=args.num_action,
                            action_dim=10,  # args.action_dim,
                            n_pose_steps=args.num_pose,
                            pose_dim=args.pose_dim,
    ...
    with torch.inference_mode():
        prev_width = None
        reset_func(wrap_model)

        for t in range(args.max_steps):
            if t % args.num_inference_step == 0:
                # pre-process inputs
                colors, depths = agent.get_observation()
                obs, cloud = create_input(colors, depths, INTRINSICS[cam_id], projector, cam_id, args.voxel_size) # cloud for open3d visualization
                
                # Force an update of the observation at the first frame
                # if len(wrap_model.env_runner.obs) == 0:
                wrap_model.update_obs(obs)

                # predict
                pred_raw_action = wrap_model.get_action()  # (horizon, action_dim)
            ...
            # colors, depths = agent.get_observation()
            # obs, _ = create_input(colors, depths, INTRINSICS[cam_id], projector, cam_id, args.voxel_size)
            # wrap_model.update_obs(obs)
