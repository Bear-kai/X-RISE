...
# add: visualization of point cloud and pose
...
class Base_Task(gym.Env):
    ...
    def load_camera(self, **kwags):
        ...
        Add cameras and set camera parameters
            - Including four cameras: left, right, front, head. --> 还有1个observer_camera和2个world_camera
        """
    ...
    def get_obs(self):
        ...
        # get observer rgb and depth
        if self.data_type.get("third_view", False):
            third_view_rgb = self.cameras.get_observer_rgb()
            # pkl_dic["third_view_rgb"] = third_view_rgb
            # add:
            tmp_dict = {'cam2world_gl': self.cameras.observer_camera.get_model_matrix(), 
                        'intrinsic_cv': self.cameras.observer_camera.get_intrinsic_matrix(), 
                        'rgb': third_view_rgb,
                        'depth': self.cameras.get_observer_depth(),
                        }
            pkl_dic['observation']['observer_camera'] = tmp_dict
        ...
        # pointcloud --> from head camera, or from the combination of front/left/right/head camera
        if self.data_type.get("pointcloud", False):
            pkl_dic["pointcloud"] = self.cameras.get_pcd(self.data_type.get("combine", False))

        # --------------------------------
        # add：target object pose
        target_obj1_pose = self.target_obj1.get_pose().to_transformation_matrix()
        target_obj2_pose = self.target_obj2.get_pose().to_transformation_matrix() if self.target_obj2 else None
        target_obj3_pose = self.target_obj3.get_pose().to_transformation_matrix() if self.target_obj3 else None
        pkl_dic["observation"]["target_obj1_pose"] = target_obj1_pose
        if target_obj2_pose is not None:
            pkl_dic["observation"]["target_obj2_pose"] = target_obj2_pose
        if target_obj3_pose is not None:
            pkl_dic["observation"]["target_obj3_pose"] = target_obj3_pose
        
        # handle articulated objects
        if self.task_name in ["open_laptop", "open_microwave"]:
            contact_idx   = 1
            contact_pose1 = self.target_obj1.get_contact_point(contact_idx, "matrix")
            pkl_dic["observation"]["target_obj1_pose"] = contact_pose1

        # --------------------------------
        # add：visualization of point cloud and pose
        if vis_pcd_flag:
            world_pcd = self.cameras.get_world_pcd()
            world_rgb_dict = self.cameras.get_world_rgb()
            head_cam_pose  = self.cameras.static_camera_list[self.cameras.head_camera_id].get_pose().to_transformation_matrix() # shape (4,4), sapien_cam to world --> refer to assets/embodiments/xxx/config.yml 
            # head_cam_pose_cv = pkl_dic['observation']['head_camera']['extrinsic_cv']  # shape (3,4), something wrong ?!
            head_cam_pose_gl = pkl_dic['observation']['head_camera']['cam2world_gl']    # shape (4,4), opengl_cam to world
            head_cam_pose_mycv = head_cam_pose_gl * np.array([1,-1,-1,1])               # shape (4,4), opencv_cam to world
            head_cam_intrinsic = pkl_dic['observation']['head_camera']['intrinsic_cv']  # h/2/tan(fovy/2) = 120/math.tan(37/2/180*math.pi)
            observer_cam_pose_gl = pkl_dic['observation']['observer_camera']['cam2world_gl']
            observer_cam_pose_mycv = observer_cam_pose_gl * np.array([1,-1,-1,1])

            def _vec_to_mat(vec):
                endpose_arr = np.eye(4)
                endpose_arr[:3, 3] = vec[:3]
                endpose_arr[:3, :3] = t3d.quaternions.quat2mat(vec[3:])
                return endpose_arr

            # choose the point cloud and camera pose to show
            which_pcd = pkl_dic["pointcloud"]  # world_pcd  # 
            which_cam_pose = head_cam_pose_mycv # observer_cam_pose_mycv # head_cam_pose_gl  # head_cam_pose  #  

            # vis contact pose
            vis_contact_pose = False # True
            if vis_contact_pose:
                contact_idx   = 1  #  4 # 
                contact_pose1 = self.target_obj1.get_contact_point(contact_idx, "matrix")
                contact_frame_pcd, contact_frame_color = gen_obj_frame_colored_pcd(contact_pose1[:3,3], contact_pose1[:3,:3], scale=0.25)
                
            # vis world, cam and objs pose
            left_endpose_mat  = _vec_to_mat(left_endpose)   # left_endpose is the return of self.robot.get_left_ee_pose()
            right_endpose_mat = _vec_to_mat(right_endpose)
            obj_frame_pcd, obj_frame_color = gen_obj_frame_colored_pcd(target_obj1_pose[:3,3], target_obj1_pose[:3,:3], scale=0.25)
            if target_obj2_pose is not None:
                obj_frame_pcd2, obj_frame_color2 = gen_obj_frame_colored_pcd(target_obj2_pose[:3,3], target_obj2_pose[:3,:3], scale=0.25)
                obj_frame_pcd = np.concatenate([obj_frame_pcd, obj_frame_pcd2], axis=0)
                obj_frame_color = np.concatenate([obj_frame_color, obj_frame_color2], axis=0)
            if target_obj3_pose is not None:
                obj_frame_pcd3, obj_frame_color3 = gen_obj_frame_colored_pcd(target_obj3_pose[:3,3], target_obj3_pose[:3,:3], scale=0.25)
                obj_frame_pcd = np.concatenate([obj_frame_pcd, obj_frame_pcd3], axis=0)
                obj_frame_color = np.concatenate([obj_frame_color, obj_frame_color3], axis=0)
            left_end_pcd, left_end_color   = gen_obj_frame_colored_pcd(left_endpose_mat[:3,3], left_endpose_mat[:3,:3], scale=0.25)
            right_end_pcd, right_end_color = gen_obj_frame_colored_pcd(right_endpose_mat[:3,3], right_endpose_mat[:3,:3], scale=0.25)
            cam_pcd, cam_color   = gen_obj_frame_colored_pcd(which_cam_pose[:3,3], which_cam_pose[:3,:3], scale=0.25)
            vis_cloud = np.concatenate([which_pcd[:,:3], obj_frame_pcd, left_end_pcd, right_end_pcd, cam_pcd])
            vis_color = np.concatenate([which_pcd[:,-3:]*255, obj_frame_color, left_end_color, right_end_color, cam_color]).astype(np.int64)
            
            left_tcp_pose_mat = _vec_to_mat(self.robot.get_left_tcp_pose())
            left_orig_endpose_mat = _vec_to_mat(self.robot.get_left_orig_endpose())
            left_tcp_pcd, left_tcp_color = gen_obj_frame_colored_pcd(left_tcp_pose_mat[:3,3], left_tcp_pose_mat[:3,:3], scale=0.30)
            left_orig_pcd, left_orig_color = gen_obj_frame_colored_pcd(left_orig_endpose_mat[:3,3], left_orig_endpose_mat[:3,:3], scale=0.35)
            vis_cloud = np.concatenate([vis_cloud, left_tcp_pcd, left_orig_pcd])
            vis_color = np.concatenate([vis_color, left_tcp_color, left_orig_color])

            if not vis_contact_pose:
                visualize(vis, vis_cloud, win=1, opts={'markersize': 2, 'markercolor': vis_color, 'title': 'robotwin_point_cloud', })
            else:
                vis_cloud = np.concatenate([vis_cloud, contact_frame_pcd])
                vis_color = np.concatenate([vis_color, contact_frame_color])
                visualize(vis, vis_cloud, win=2, opts={'markersize': 2, 'markercolor': vis_color, 'title': 'robotwin_contact_frame', })
                
            img_pil = Image.fromarray(pkl_dic["observation"]["front_camera"]["rgb"])
            img_pil.save('vis_front_camera.png')
            img_pil = Image.fromarray(pkl_dic["observation"]["head_camera"]["rgb"])
            img_pil.save('vis_head_camera.png')
            img_pil = Image.fromarray(world_rgb_dict["world_camera1"])
            img_pil.save('vis_world_camera1.png')
            img_pil = Image.fromarray(third_view_rgb)
            img_pil.save('vis_third_view.png')
    ...
    def merge_pkl_to_hdf5_video(self):
        ...
        target_video_path = f"{self.save_dir}/video/episode{self.ep_num}.mp4"   # head view 
    ...
    def together_move_to_pose(
        ...
        """ --> left和right是只规划，together是规划并执行！故前两者依赖take_dense_action()，后者不需要！
        Interpolative planning with screw motion.
        Will not avoid collision and will fail if the path contains collision.
        """
        if not self.plan_success:
    ...
    def choose_best_pose(self, res_pose, center_pose, arm_tag: ArmTag = None):
        ...
        for i in range(pose_num):
            if traj_lst["status"][i] != "Success":
                continue
            if now_pose is None or len(traj_lst["position"][i]) < now_step:     # bug?! now_step always -1 ?
    ...
    def place_actor(
        ...
        **args,
        ...
        if self.need_plan:
            ...
            place_pre_pose = [0, 0, 0, 0, 0, 0, 0]      # placeholder, will be replaced by the pre-planned pose
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee', 'delta_ee']='qpos'):  # for evaluation！ # action_type: qpos or ee
        ...
        left_gripper_path = np.hstack((left_current_gripper, left_gripper_actions))     # “current;target”，for path planning
