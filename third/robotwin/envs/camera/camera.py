...
class Camera:
    ...
    def __init__(self, bias=0, random_head_camera_dis=0, **kwags):
        # kwags: refer to both demo_clean.yml and assets/embodiments/aloha-agilex/config.yml, see collect_data.py
    ...
    def get_config(self) -> dict:
        ...
        def _get_config(camera):
            ...
            return {
                ...
                # "extrinsic_cv": camera_extrinsic_cv,  # Comment out. Something wrong based on the visualization in _base_task.py ?!
    ...
    def get_observer_depth(self):
        ...
        depth = _get_depth(self.observer_camera)

        return depth

    # add
    def get_world_rgb(self):
        ...
        result = {
            "world_camera1": _get_rgb(self.world_camera1),
            "world_camera2": _get_rgb(self.world_camera2),
        }
    ...
    def get_world_pcd(self):                # no point cropping 
        self.world_camera1.take_picture()
        self.world_camera2.take_picture()
        ...
        return res_pcd[index]  # pcd_array

    # Get Camera PointCloud
    def get_pcd(self, if_combine=False):    # do point cropping
        ...
        combined_pcd = np.array([])  # bug: This requires `collect_wrist_camera=True`，otherwise vstack will raise error. Fixed by if！
        ...
        if if_combine:
            ...
            for camera, camera_name in zip(self.static_camera_list, self.static_camera_name):
                if camera_name == "head_camera":
                    if self.collect_head_camera:
                        combined_pcd = np.vstack((combined_pcd, _get_camera_pcd(camera))) if len(combined_pcd) > 0 else _get_camera_pcd(camera)
                else:
                    combined_pcd = np.vstack((combined_pcd, _get_camera_pcd(camera))) if len(combined_pcd) > 0 else _get_camera_pcd(camera)
